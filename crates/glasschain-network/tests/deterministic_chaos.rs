...
//! Chaos scenarios for the node: mining budgets, crash/rejoin, watcher stress,
//! SNCM nudges, event-bus ordering, and application-layer partition reference
//! behaviour. They run on the real Tokio runtime.
//!
//! For network-level partitions over a simulated transport, see
//! `tests/turmoil_chaos.rs` (turmoil; decision #154).
//!
//! ## Running
//!
//! ```bash
//! cargo test -p glasschain-network --test deterministic_chaos
//! ```
...
fn inv_tx(owner: &str, delta: i64) -> Transaction {
    Transaction::new(TransactionKind::InventoryUpdate(InventoryUpdate {
        ...
        reason: "deterministic chaos test".into(),
    }))
}
...
/// window, so the test never flakes due to scheduling jitter.
#[tokio::test]
async fn test_single_node_mines_within_time_budget() {
    let addr = free_addr();
    let node = Node::new("chaos-node-1", &addr, 1);
...
async fn test_partition_merge_converges_with_final_commits() {
...
/// `WatcherService` inside one wall-clock budget.
///
/// This validates the Phase 4 plan goal: "handle 1 000+ autonomous inventory
/// triggers per second." The `WatcherService` is synchronous, so the firings
/// complete in milliseconds.
#[tokio::test]
async fn test_1000_autonomous_triggers_stress() {
    const N: usize = 1_000;
...
async fn test_sncm_nudge_under_simulated_load() {
    let addr = free_addr();
    ...
    let compliant_asset = TraceableAsset {
        ...
        batch_number: Some("LOTE-CHAOS-001".into()),
        expiry_date: Some("2028-06-30".into()),
        serial_number: Some("SN-CHAOS-001".into()),
...
#[tokio::test]
async fn test_event_bus_ordering_under_simulated_time() {
...
/// Partition convergence at the application layer: nodes that cannot dial each
/// other diverge, and a node connecting to one side adopts that side's chain.
///
/// Network-level partitions over a simulated transport live in
/// `tests/turmoil_chaos.rs` (turmoil; decision #154).
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_partition_reference_implementation() {
