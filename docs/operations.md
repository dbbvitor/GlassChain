...
| `verus` | ubuntu | `cargo verus verify -p glasschain-vm -p glasschain-core -p glasschain-identity --all-features --locked` — production-form proofs for the gas arithmetic, the BFT quorum/bitmap kernels + bitmap expansion + certificate admission, the consensus-round kernels (`glasschain-core/src/rounds.rs`), the TOFU pin decision (`glasschain-core/src/pin.rs`), the trust-score arithmetic (`glasschain-core/src/asset.rs`), the MSP height-window authorization (`glasschain-identity/src/msp_policy.rs`) and channel membership + the private-payload gate (`glasschain-identity/src/channel.rs`); Verus `0.2026.09.24.b9416fa` from the pinned rolling-release zip. A cheat-marker grep (bare `assume(`/`admit(`, `external_body`, `axiom`) runs first so proofs cannot pass vacuously. Warm seconds, 30-minute timeout |
...
`cargo fmt`, coverage (the Codecov project gate needs the full report),
ASan/LSan (the `-Zbuild-std` build dominates, so scoping the test list saves
nothing), snarf and deny/machete stay whole-workspace by nature.
